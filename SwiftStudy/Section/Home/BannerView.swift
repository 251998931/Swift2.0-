...
/// 轮播图 每次滚动的距离
private let bannerWidth = ScreenWidth
...
class BannerView: UIView,UIScrollViewDelegate {
    ...
    {
        showScrollView.frame = CGRectMake(0, 0, ScreenWidth, 324/2)
        ...
        //利用 n+2 方法 index=0 前面加最后一张图片     index = count-1 后加第一张图片
            //最后一张放到index = 0的位置
        let lastImageView = UIImageView.init(frame: CGRectMake(0, 0, ScreenWidth, 324/2))
        ...
        let firstImageView = UIImageView.init(frame: CGRectMake(CGFloat(bannerArray.count+1)*ScreenWidth , 0, ScreenWidth, 324/2))
        ...
        firstImageView.image = UIImage(named: "SignInBackImage")
        lastImageView.image = UIImage(named: "PersonCenterbackImage")
        ...
        for(var index = 0 ;index < self.bannerArray.count ;index++)
        {
            ...
            let imageView = UIImageView(frame: CGRectMake(ScreenWidth*CGFloat(index+1), 0, ScreenWidth, 324/2))
            ...
            if index%2==0 {
                ...
                imageView.image = UIImage(named: "SignInBackImage")
 
            }else {
                imageView.image = UIImage(named: "PersonCenterbackImage")
            }
            
        }
        showScrollView.contentSize = CGSizeMake(ScreenWidth*CGFloat(bannerArray.count+2), 0)
        showScrollView.contentOffset = CGPointMake(ScreenWidth, 0)

        self.addSubview(showScrollView)
//        showScrollView.backgroundColor = UIColor.cyanColor()
        ...
        pageControl.center = CGPointMake(ScreenWidth/2, pageControl.center.y)
    ...
    {
        //滑动到最后一张
        if(showScrollView.contentOffset.x == CGFloat(bannerArray.count+1)*ScreenWidth)
        {
            showScrollView.contentOffset = CGPointMake(ScreenWidth, 0)
            pageControl.currentPage = 0
        }else
        {
            let page = Int( (showScrollView.contentOffset.x-ScreenWidth) / ScreenWidth)
    ...
    //如果滑到 第一张
        ...
        {
            showScrollView.contentOffset = CGPointMake(CGFloat(bannerArray.count)*ScreenWidth, 0)
            pageControl.currentPage = bannerArray.count-1
        }else if(showScrollView.contentOffset.x == CGFloat(bannerArray.count+1)*ScreenWidth)
        {
            //滑倒最后一张的时候
            showScrollView.contentOffset = CGPointMake(ScreenWidth, 0)
            pageControl.currentPage = 0
